...
export default function FilmsPage() {
  ...
  const filteredFilms = useMemo(() => {
    if (activeGenreId === null) return films;
    return films.filter((m) => (m.genres ?? []).some((g) => g.genre.id === activeGenreId));
  }, [films, activeGenreId]);
  ...
  const groups = useMemo(() => groupByLetter(sortedFilms), [sortedFilms]);
    ...
    <div>
      ...
      <div className="px-4 md:px-8 pt-8 pb-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <h1 className="text-3xl font-black tracking-tight">Films</h1>
          {!isLoading && (
            <span className="text-sm text-zinc-400 bg-zinc-800/80 border border-zinc-700/50 px-2.5 py-0.5 rounded-full tabular-nums">
              ...
              {activeGenreId !== null && (
                <span className="text-zinc-600"> / {films.length}</span>
              )}
            </span>
          )}
        </div>
      ...
      <div className="px-4 md:px-8 py-6">
        {isLoading ? (
          ...
          <p className="text-zinc-600 text-sm py-12 text-center">Aucun film dans cette catégorie.</p>
        ) : viewMode === 'recent' ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {filteredFilms.map((m) => (
